...
public class OnlineObstacles : MonoBehaviour
{
    Rigidbody2D playerRb;

    OnlineUIScript _uiScript;

    OnlinePowerUp _powerUp;
    ...
    private void Start()
    {
        ...
        _uiScript = GameObject.Find("UI Handler").GetComponent<OnlineUIScript>();
        _powerUp = GetComponent<OnlinePowerUp>();
        _shakeScript = GetComponent<ShakeyCam>();
        _moveScript = GetComponent<OnlineMovementScript>();
        _switchScript = GetComponent<OnlinePlayerSwitch>();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.transform.parent != null && collision.transform.parent.gameObject.tag == "Obstacle")
        {
            ...
            if (_powerUp.powerUps != OnlinePowerUp.PowerUps.Speed)
            {
        ...
        if(collision.gameObject.tag == "Bounds")
        {
            ...
            if (_moveScript.isJumping)
                {
                    if (_moveScript.playerPositions == OnlineMovementScript.PlayerPositions.Bot)
                    {
                        _moveScript.playerPositions = OnlineMovementScript.PlayerPositions.Top;
                    }
                    else
                    {
                        _moveScript.playerPositions = OnlineMovementScript.PlayerPositions.Bot;
                    }
                }
